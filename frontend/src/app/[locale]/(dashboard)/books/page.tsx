...
import { getPubBooks } from '@/actions/books.actions';
import { PubBook, PubBooksResponse } from '@/types/pub_book';
...
export default async function Home({ params, searchParams }: HomePageProps) {
  ...
  const response = await getPubBooks();
  ...
  if (response && 'data' in response && 'meta' in response) {
    data = (response as PubBooksResponse).data;
    meta = (response as PubBooksResponse).meta;
    ...
    <div className='flex flex-col gap-y-8 h-[calc(100vh-30px)] overflow-auto invisible-scrollbar pb-4 py-2'>
      <div className='flex items-center justify-between w-full'>
        <p className='text-2xl font-bold text-primary'>
          {translations.page.books.titleName}
        </p>
          ...
          <div>
            {!data || data.length === 0 ? (
              ...
              <div className='w-full flex flex-col justify-between h-full gap-y-6'>
                <BookList books={data as PubBook[]} />
              </div>
            )}
          </div>
        </div>
